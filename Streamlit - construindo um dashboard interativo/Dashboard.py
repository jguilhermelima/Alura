...
fig_mapa_receita = px.scatter_geo(receita_estados, lat='lat', lon='lon',
                                  scope='south america', size='Preço', template='seaborn',
...
fig_receita_categoria = px.bar(receita_categorias, text_auto=True,
                               title = 'Receita por categoria', orientation='h')
fig_receita_categoria.update_layout(yaxis_title ='Receita', yaxis=dict(autorange="reversed"))
...
fig_vendas_mensal = px.line(vendas_mensal, x= 'Mes', y= 'Preço',
                             markers= True, range_y= (0, receita_mensal.max()),
...
fig_vendas_estados = px.bar(vendas_estados.head(), x='Local da compra', y='Preço',
                             text_auto=True, title = 'Top estados (vendas)')

fig_vendas_estados.update_layout(yaxis_title ='Quantidade de vendas')


fig_vendas_categoria = px.bar(vendas_categorias, text_auto=True,
                               title = 'Quantidade de vendas por categoria', orientation='h')
fig_vendas_categoria.update_layout(yaxis_title ='Quantidade de vendas', yaxis=dict(autorange="reversed"),legend_title_text='Quantidade')


## Visualização no stream lit
...
with aba3:
    ...
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        # gráfico criado dentro da coluna, pois esta sendo utilizado a função number_input criada acima
